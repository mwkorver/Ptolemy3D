...
<!--- Checks Queue to see if any JOBs remaining, if so gets attributes, processes the deletes queue item from queue --->
 ...
 --->
			...
			<cfloop condition="cfhttpcntr LT 3">
				...
				<!--- if all the retries did not work set this job id as processed=true --->
				<cfif Find("200",cfhttp.statuscode) EQ 0>
				
					<cfquery datasource="jobqueue" username="test" password="test" name="setprocessedtrue">
						UPDATE #url.tableName# SET PROCESSED=TRUE where id=#getOneJob.id#
					</cfquery>
