...
public class Tile {
	...
	protected void display(GL gl, boolean texture, Tile rightTile,
		...
		if (DEBUG) {
			if (ProfilerInterface.forceLevel != -1
					&& ProfilerInterface.forceLevel != drawZlevel) {
				return;
			}
		}
		...
		if (levelID < /* != */(levels.length - 1)) {
			...
			if (levelBelow.isVisible()) {
				...
				if ((c_lrz < landscape.getMaxLatitude())
						&& (c_lrz > -landscape.getMaxLatitude())) {
					if (!((lrx <= c_ulx) || (ulx >= c_lrx) || (lrz <= c_ulz) || (ulz >= c_lrz))) {
						displayClipped(ulx, ulz, lrx, lrz, c_ulx, c_ulz, c_lrx,
								c_lrz);
						return;
					}

					int maxx = (landscape.getMaxLongitude() / levelBelow
							.getTileSize())
							* levelBelow.getTileSize();
					if (maxx != landscape.getMaxLongitude()) {
						maxx += levelBelow.getTileSize();
					}

					if (c_lrx > maxx) {
						int t_past = Level.LEVEL_NUMTILE_LON
								- ((landscape.getMaxLongitude() - c_ulx) / levelBelow
										.getTileSize()) - 1;
						c_ulx = -maxx;
						c_lrx = c_ulx + (t_past * levelBelow.getTileSize());

						if (!((lrx <= c_ulx) || (ulx >= c_lrx)
								|| (lrz <= c_ulz) || (ulz >= c_lrz))) {
							// if (!((lrx <= ulx) || (ulx >= c_lrx) || (lrz <=
							// c_ulz) || (ulz >= c_lrz))) { //Old (replace with
							// above line 08-2008)
							displayClipped(ulx, ulz, lrx, lrz, c_ulx, c_ulz,
									c_lrx, c_lrz);
							return;
						}
					}
	...
	private void displayClipped(int ulx, int ulz, int lrx, int lrz, int c_ulx,
	...
	protected boolean pick(double[] intersectPoint, double[][] ray) {
		...
		if (!useTin && !useDem) {
			...
			// texture ratios
			int n = (int) (((double) ((x_w > z_w) ? x_w : z_w) / (double) landscape
					.getMaxLongitude()) * 50);// precision, soon to be dem...
			if (n <= 2) {
				n = 4;
			}
			...
			for (int i = startz; i < endz; i++) {
				...
				for (int j = startx; j < endx; j++) {
					...
					double dy1 = ((dem[pos_d1] << 8) + (dem[pos_d1 + 1] & 0xFF))
							* scaler;
					double dy2 = ((dem[pos_d2] << 8) + (dem[pos_d2 + 1] & 0xFF))
							* scaler;
