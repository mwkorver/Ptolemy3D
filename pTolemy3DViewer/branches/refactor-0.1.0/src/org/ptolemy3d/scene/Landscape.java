...
public class Landscape {
	 ...
	 * @param ptolemy
	 */
	public Landscape(Ptolemy3D ptolemy) {
	...
	private final void correctLevels() {
		...
		for (int p = 0; p < levels.length; p++) {
			...
			// TODO - What???
			minlon = ((int) (camera.getPosition().getLongitudeDD() / tileWidth) * tileWidth)
					- (tileWidth * 4);
			...
			if ((tileWidth * LEVEL_NUMTILE_LAT) > (maxLatitude * 2)) {
				...
				minlat = ((int) (camera.getPosition().getLatitudeDD() / tileWidth) * tileWidth)
						+ (tileWidth * 4);
				if (minlat > toptile) {
					minlat = toptile
							- (((minlat - maxLatitude) / tileWidth) * tileWidth);
					if (minlon > 0) {
						minlon -= lon_move;
					} else {
						minlon += lon_move;
	...
	private void processVisibility() {
		...
		for (int p = levels.length - 1; p >= 0; p--) {
			...
			level
					.setStatus(((camera.getVerticalAltitudeMeters() > level
							.getMaxZoom()) || (camera
							.getVerticalAltitudeMeters() <= level.getMinZoom())) ? false
							: true);
			if (level.isStatus()) {
			...
			if (!level.isStatus()
					&& (hasVisLayer || (camera.getVerticalAltitudeMeters() > level
							.getMaxZoom()))) {
				level.setVisible(false);
				continue;
			}
	...
	protected void draw(GL gl) {
		...
		if (displayMode == DISPLAY_MESH) {
			...
			gl.glDisable(GL.GL_TEXTURE_2D);
		} else if (displayMode == DISPLAY_JP2RES
				|| displayMode == DISPLAY_TILEID
				|| displayMode == DISPLAY_LEVELID) {
			gl.glDisable(GL.GL_TEXTURE_2D);
		}
	...
	public double groundHeight(double lon, double lat, int minLevel) {
		...
		double[][] ray = { { lon, ptolemy.cameraController.maxAlt, lat },
