...
public class Ptolemy3DApplet extends Applet implements Ptolemy3DJavascript,
	 ...
	 * @param altitude
	 *            altitude, zero is the altitude of the ground with no
	 *            elevations.
	 * @param direction
	 *            unit in degrees.
	 * @param pitch
	 *            unit in degrees.
	 */
	public void setOrientation(double longitude, double latitude,
			double altitude, double direction, double pitch) {
		try {
			final Ptolemy3DUnit unit = ptolemy.unit;

			ptolemy.cameraController.setOrientation(longitude * unit.DD,
					latitude * unit.DD, altitude, direction, pitch);
		} catch (Exception e) {
	...
	public void setLongitude(double longitude) {
		try {
			...
			cameraController.setOrientation(longitude * unit.DD, camera
					.getLatitudeDD(), camera.getAltitude(), camera
					.getDirectionDegrees(), camera.getPitchDegrees());
		} catch (Exception e) {
			IO.printStackJavascript(e);
		}
	}
	...
	public void setLatitude(double latitude) {
		try {
			...
			cameraController.setOrientation(camera.getLongitudeDD(), latitude
					* unit.DD, camera.getAltitude(), camera
					.getDirectionDegrees(), camera.getPitchDegrees());
		} catch (Exception e) {
			IO.printStackJavascript(e);
		}
	...
	/**
	 * Set the view altitude, zero is the altitude of the ground with no
	 * elevation.<BR>
	 */
	public void setAltitude(double altitude) {
		try {
			...
			cameraController.setOrientation(camera.getLongitudeDD(), camera
					.getLatitudeDD(), altitude, camera.getDirectionDegrees(),
					camera.getPitchDegrees());
		} catch (Exception e) {
			IO.printStackJavascript(e);
		}
	}
	...
	/**
	 * @return the altitude, zero is the altitude of the ground with no
	 *         elevation.
	...
	/**
	 * @return the altitude of the view, in meters, from the zero altitude
	 *         reference.<BR>
	...
	public void setDirection(double direction) {
		try {
			...
			cameraController.setOrientation(camera.getLongitudeDD(), camera
					.getLatitudeDD(), camera.getAltitude(), direction, camera
					.getPitchDegrees());
		} catch (Exception e) {
			IO.printStackJavascript(e);
		}
	 ...
	 * 
	 * @param lon
	 *            latitude, unit is degrees.
	 * @param lat
	 *            longitude, unit is degrees.
	 * @param alt
	 *            altitude, zero is the altitude of the ground with no
	 *            elevation.
	 * @see Landscape#DD
	 * @see #flyStop() FIXME Is the altitude in meters .
	 */
	...
	public void flyTo(double lon, double lat, double alt, int speed,
	...
	public final void flyToPositionTime(double lon, double lat, double alt,
			...
			double cruise_tilt, double time) {
		try {
			final Ptolemy3DUnit unit = ptolemy.unit;

			ptolemy.cameraController.flyToPositionTime(lon * unit.DD, lat
					* unit.DD, alt, s_direction, s_tilt, flight_arc,
					cruise_tilt, time);
		} catch (Exception e) {
			IO.printStackJavascript(e);
		}
	...
	/* Speed flight */
	public final void flyToPositionSpeed(double lon, double lat, double alt,
			double s_direction, double s_tilt, double flight_arc,
			double cruise_tilt, double speed) {
		try {
			final Ptolemy3DUnit unit = ptolemy.unit;

			ptolemy.cameraController.flyToPositionSpeed(lon * unit.DD, lat
					* unit.DD, alt, s_direction, s_tilt, flight_arc,
					cruise_tilt, speed);
		} catch (Exception e) {
			IO.printStackJavascript(e);
	 ...
	 * @see #flyTo(double, double, double, double, double, double, double,
	 *      double)
	 */
	public void flyStop() {
		try {
	...
	public String pluginAction(int pluginIndex, String commandName,
			String commandParams) {
		try {
			String ret = ptolemy.scene.plugins.pluginAction(pluginIndex,
	...
	/**
	 * Change landscape display mode. Display modes: 0 Default display mode.
	 * (Landscape.DISPLAY_STANDARD) 1 Landscape geometry is rendered in line
	 * mode, to visualize tile triangle geometry. (Landscape.DISPLAY_MESH) 2
	 * Color of the landscape is the elevation. (Landscape.DISPLAY_SHADEDDEM) 3
	 * Tile color is the jp2 current resolution. (Landscape.DISPLAY_JP2RES) 4
	 * Tile color is the tile id. (Landscape.DISPLAY_TILEID) 5 Tile color is the
	 * tile id. (Landscape.DISPLAY_LEVELID)
	 * 
	 * @see Landscape#DISPLAY_STANDARD
	 * @see Landscape#DISPLAY_MESH
	 * @see Landscape#DISPLAY_SHADEDDEM
	 * @see Landscape#DISPLAY_JP2RES
	 * @see Landscape#DISPLAY_TILEID
	 * @see Landscape#DISPLAY_LEVELID
	 */
	...
	/**
	 * Set terrain elevation scaler.<BR>
	 * Default value is 1.
	 */
	...
	/**
	 * @return the current landscape terrain elevation state (true means
	 *         elevation enable, false measn no elevation).
	 */
	...
	/**
	 * Set the behavior of the mouse clicked event. "0" Display coordinates of
	 * the position picked on the terrain and call the javascript
	 * <i>plotPosition</i> function. "1" Move to the picked position on the
	 * terrain. "2" Move and zoom to the picked position on the terrain.
	 */
	...
	public final void addCursor(String server, String file, String name) {
		...
		Image cimg = null;
		try {
			cimg = Toolkit.getDefaultToolkit().getImage(
					new URL("http://" + server + file));
		} catch (Exception e) {
			IO.printStackJavascript(e);
		} finally {
			curVec.put(name, Toolkit.getDefaultToolkit().createCustomCursor(
					cimg, new Point(0, 0), name));
		}
	}

	/**
	 * Follow the terrain elevation, ie fly at a constant altitude from the
	 * terrain ("true"); Or, fly at a constant altitude from the zero altitude
	 * reference ("false"). Default value is "false"
	 */
	...
	public final void panorama(double tx, double tz, double camht, int rtimes,
			double rspeed, double rpitch) {
		try {
			ptolemy.cameraController.panorama(tx, tz, camht, rtimes, rspeed,
					rpitch);
		} catch (Exception e) {
			IO.printStackJavascript(e);
		}
	...
	public final void setServerParameters(String server_index, String ip,
			String key, String jp2loc, String demloc, String urlappend) {
		int serverindex = -1;
		try {
		...
		if ((serverindex >= 0)
				&& (serverindex < ptolemy.configuration.dataServers.length)) {
			...
			if (jp2loc == null) {
				...
				ptolemy.configuration.locations[serverindex] = new String[ptok
						.countTokens()];
				for (int nfloc = 0; nfloc < ptolemy.configuration.locations[serverindex].length; nfloc++) {
					ptolemy.configuration.locations[serverindex][nfloc] = addTrailingChar(
							ptok.nextToken(), "/");
				}
			}
			...
			if (demloc == null) {
				...
				ptolemy.configuration.DEMLocation[serverindex] = new String[ptok
						.countTokens()];
				for (int nfloc = 0; nfloc < ptolemy.configuration.DEMLocation[serverindex].length; nfloc++) {
					ptolemy.configuration.DEMLocation[serverindex][nfloc] = addTrailingChar(
							ptok.nextToken(), "/");
				}
			}
			canvas.repaint();
	...
	/**
	 * Acceleration/Rotation scaler. Default value is 1.
	 */
	...
	public boolean checkFlightStatus() {
		if ((ptolemy.cameraController.isActive)
				|| (ptolemy.cameraController.inAutoPilot > 0)) {
			return true;
		} else {
			return false;
