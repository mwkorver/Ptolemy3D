...
public class Jp2Tile {
    ...
    protected final void set(int lvl, int scale, int lon, int lat) {
        ...
        if (levels != null) {
            final int divider = levels[lvl].getDivider();
        ...
        fileBase += Ptolemy3D.getConfiguration().area + "x" + lon + "y" + lat;

        final QuadBufferPool quadBufferPool = Ptolemy3D.getTileLoader().quadBufferPool;
    ...
    protected String fillFile(int res) {
        ...
        return Ptolemy3D.getConfiguration().backgroundImageUrl + "&lat=" + lat + "&lon=" + lon + "&scale=" + scale + "&w=" + tileLoader.twidth[res] + "&h=" + tileLoader.twidth[res] + "&res=" + res;
    }
