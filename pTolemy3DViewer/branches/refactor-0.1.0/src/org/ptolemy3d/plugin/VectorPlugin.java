...
public class VectorPlugin implements Plugin
{
	...
	public void init(Ptolemy3D ptolemy)
	{
		...
		this.interp_len = ptolemy.scene.landscape.getLevels()[0].getTileSize();
			...
			if (parameters.substring(0, 1).equals("P"))
			{
				...
				while (ptok.hasMoreTokens())
				{
					try
					{
						...
						{
							DisplayRadius = (int) (Integer.parseInt(ptok.nextToken()) * unit.getCoordSystemRatio());
						}
						...
						{
							VECTOR_RAISE = Float.parseFloat(ptok.nextToken()) * unit.getCoordSystemRatio();
						}
...
//			labelPart.setPluginParameters("P -status " + ((STATUS) ? "1" : "0") + " -jspquery 0 -numicons 1 -iconprefix /JetStreamLib/common/icon/bdot -mindistance 65 -encoding " + TextLabelENC + " -fontcolor " + TextLabelFontColor + "  -bgcolor " + TextLabelBgColor + " -minalt " + DISP_MINZ + " -maxalt " + DISP_MAXZ + " -position RM -mindistance " + TextLabelMD + "");
	...
	private final void interpolateVectors()
	{
		...
		double approxht = landscape.groundHeight(position.getLongitudeDD(), position.getLatitudeDD(), 0) / unit.getCoordSystemRatio();
		...
		for (n = (START_INTP_LVL - 1); n >= 0; n--)
		{
			...
			for (i = 0; i < numgeoms; i++)
			{
				if (!isFixed[i][n])
				{
					dist = (float) Math3D.distance2D(position.getLongitudeDD(), ctds[i][ 0], position.getLatitudeDD(), ctds[i][ 1]);
					if (dist < closest)
					{
						closest = dist;
						fix_id = i;
	...
	private void setParamsXSLT(){
		...
		VECTOR_RAISE = Float.parseFloat(xsltOutput.getVectorRaise()) * ptolemy.unit.getCoordSystemRatio();
...
//			int ulz = (int) unit.virtualY(feature_maxz);
	...
	public String pluginAction(String commandname, String command_params)
	{
		try
		{
			...
			{
				return "vector," + STATUS + "," + QueryWidth + "," + (DisplayRadius / unit.getCoordSystemRatio()) + "," + DISP_MAXZ + "," + DISP_MINZ + "," + (VECTOR_RAISE / unit.getCoordSystemRatio()) + "," +
			...
			else if (commandname.equalsIgnoreCase("setInfo"))
			{
				...
				DisplayRadius = (int) (Float.parseFloat(stok.nextToken()) * unit.getCoordSystemRatio());
				DISP_MAXZ = Integer.parseInt(stok.nextToken());
				DISP_MINZ = Integer.parseInt(stok.nextToken());
				VECTOR_RAISE = Float.parseFloat(stok.nextToken()) * unit.getCoordSystemRatio();
	...
	public boolean onPick(double[] intersectPoint)
	{
		...
		double tx, ty;
		{
			...
			ty = out[0] * unit.getDD(); // y
			tx = out[1] * unit.getDD(); // x
		}
		for (i = 0; i < numgeoms; i++)
		{
