	            ...
	            for(int  n=0; n<nTokens; n++){
	                ...
	                try {
	                    ...
	                    if (".PGM".equalsIgnoreCase(inext)) { // PGM file
	                        ...
	                        ncomp += 3;
/** START OF UPGRADE */
	                    } else if (ImgReaderImageIO.isSupported(inext.replace(".", ""))) { // PPM file
	                        if (ncomp > 0) {
	                            error("With PNG input format only 1 input "+
	                                  "file can be specified",2);
	                            return;
	                        }
	                        imreadervec.addElement(new ImgReaderImageIO(infile));
	                        ppminput = true;
	                        ncomp += 3;
/** END OF UPGRADE */
	                    } else { // Should be PGX
				...
				imreadervec.addElement(new ImgReaderImageIO(bufferedImage));
				ppminput = true;
				ncomp += 3;
            }
