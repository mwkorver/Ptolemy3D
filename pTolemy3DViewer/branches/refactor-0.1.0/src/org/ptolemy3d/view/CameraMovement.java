...
public class CameraMovement {
	...
	protected final void setCamera(Matrix16d modelView) throws Exception {
		...
		isActive = (inputs.key_state != 0) || (velocity != 0)
				|| (vert_velocity != 0) || (ud_rot_velocity != 0)
				|| (lr_rot_velocity != 0) || (horz_velocity != 0);
		...
		if (inAutoPilot > 0) {
			...
			if (fmode == 0) {
				...
				mmx = accel = (int) ((2 * (Math.tan(0.5235987756) * (camera
						.getPosition().alt + 1))) * 25 * relspdmult);
			...
			{
				camera.getPosition().alt += velocity * scaler; // distance away
																// from
																// surface...
				...
				double rot_y = (lr_rot_velocity * rot_vel_scaler / 10)
						* Math3D.DEGREE_TO_RADIAN_FACTOR;
				...
				if (fmode == 0) {
					...
					horz_velocity /= (2 * (0.57735026919189393373967470078005 * camera
							.getPosition().alt));
					vert_velocity /= (2 * (0.57735026919189393373967470078005 * camera
							.getPosition().alt));
					rot_y = (((0.57735026919189393373967470078005 * camera
							.getPosition().alt) / GL_WIDTH)
							* lr_rot_velocity / 10)
				...
				if (desiredTilt == -1) {
					...
					if (Math.abs(tiltDiff) < desiredTiltIncrement) {
						...
						camera
								.setTilt(camera.getTilt()
										+ (desiredTilt > camera.getTilt() ? desiredTiltIncrement
		...
		ground_ht = landscape.groundHeight(camera.getPosition().lon, camera
				.getPosition().lat, 0);
...

		{
			if (!checkAlt() && (inAutoPilot == 0)) {
				...
				if (vpPos_1_bak > camera.getPosition().alt) // always allow to
															// zoom out
				{
			...
			camera.getPosition().lat = Math.asin(camera.vpMat.m[1][2])
					* Math3D.RADIAN_TO_DEGREE_FACTOR;
			camera.getPosition().lon = Math3D.angle2dvec(-1, 0,
					camera.vpMat.m[2][2], camera.vpMat.m[0][2], true);
			...
			double yup_x = (Math.sin((Math3D.DEGREE_TO_RADIAN_FACTOR * camera
					.getPosition().lon)) * Math
					.sin((camera.getPosition().lat * Math3D.DEGREE_TO_RADIAN_FACTOR)));
			double yup_y = Math.cos((Math3D.DEGREE_TO_RADIAN_FACTOR * camera
					.getPosition().lat));
			double yup_z = (Math.cos((Math3D.DEGREE_TO_RADIAN_FACTOR * camera
					.getPosition().lon)) * Math
					.sin((camera.getPosition().lat * Math3D.DEGREE_TO_RADIAN_FACTOR)));
			...
			if ((camera.getPosition().lon > (90 * unit.getDD()))
					|| (camera.getPosition().lon < (-90 * unit.getDD()))) {
				if (((yup_x * camera.vpMat.m[1][1]) - (yup_y * camera.vpMat.m[0][1])) < 0) { // rotate
																								// left
					camera.setDirection(Math3D.TWO_PI_VALUE
							- camera.getDirection());
				}
			} else {
				if (((yup_x * camera.vpMat.m[1][1]) - (yup_y * camera.vpMat.m[0][1])) > 0) { // rotate
	...
	private final void setCameraMatrix(boolean addGroundHeight) {
		...
		camera.cameraPos[0] = (camera.vpMat.m[0][2] * (EARTH_RADIUS + groundH))
				+ (camera.cameraMat.m[0][2] * camera.getPosition().alt);
		camera.cameraPos[1] = (camera.vpMat.m[1][2] * (EARTH_RADIUS + groundH))
				+ (camera.cameraMat.m[1][2] * camera.getPosition().alt);
		camera.cameraPos[2] = (camera.vpMat.m[2][2] * (EARTH_RADIUS + groundH))
				+ (camera.cameraMat.m[2][2] * camera.getPosition().alt);

		camera.setVertAltDD(Math
				.sqrt((camera.cameraPos[0] * camera.cameraPos[0])
						+ (camera.cameraPos[1] * camera.cameraPos[1])
						+ (camera.cameraPos[2] * camera.cameraPos[2]))
				- EARTH_RADIUS);
	...
	private final boolean checkAlt() {
...

		{
			...
			camera.setCameraY(Math.asin(camera.cameraPos[1] / mag)
					* Math3D.RADIAN_TO_DEGREE_FACTOR * unit.getDD());
			camera.setCameraX(Math3D.angle2dvec(-1, 0, camera.cameraPos[2]
					/ mag, camera.cameraPos[0] / mag, true)
					* unit.getDD());
	...
	protected synchronized final void updatePosition(boolean force) {
		...
		if ((inputs.key_state == 0) && (inAutoPilot == 0) && (!force)
	...
	public final void setOrientation(Position latLonAlt, double dir, double pit) {
		...
		try {
			...
			rotMat.rotY(((lon / unit.getDD()) + 180)
					* Math3D.DEGREE_TO_RADIAN_FACTOR);
	...
	public final void setFollowDem(boolean v) {
		...
		if (prev && !followDemOn) {
			double ht = landscape.groundHeight(camera.getPosition().lon, camera
					.getPosition().lat, 0);
	...
	private final void flyToPositionDD(Position latLonAlt, double s_direction,
		...
		if (Math3D.distance2D(camera.getPosition().lon, lon, camera
				.getPosition().lat, lat) < 10) {
			lon += 7;
			lat += 7;
		}
		...
		cruise_alt = (((EARTH_RADIUS * angle) * Math.atan(flight_arc
				* Math3D.DEGREE_TO_RADIAN_FACTOR)) + auto_y);
	...
	private final void moveAutopilot() {
		...
		if (rot_lr_trav != angle) {
			...
			if (rot_lr_trav < halfway) {
				camera.getPosition().alt = ((Math
						.sin(((Math.PI * rot_lr_trav) / angle)) * (cruise_alt - start_y)) + start_y);
			} else {
				camera.getPosition().alt = ((Math
						.sin(((Math.PI * rot_lr_trav) / angle)) * (cruise_alt - auto_y)) + auto_y);
			}
			if (camera.getPosition().alt > maxAlt) {
				camera.getPosition().alt = maxAlt;
			}
	...
	private final void flyToDD(Position latLonAlt, int speed, int ld_angle,
		...
		cruise_alt = auto_y
				+ ((type == 0) ? (EARTH_RADIUS * angle)
						* (Math
								.atan(arc_angle
										* Math3D.DEGREE_TO_RADIAN_FACTOR)) : 0);
		if (cruise_alt > 7000000) {
			cruise_alt = 7000000;
		}
		// stopdirection = 0;
	...
	public void panorama(Position latLonAlt, int rtimes, double rspeed,
...

		{
			...
			halfway = (Math.sqrt((camera.cameraPos[0] * camera.cameraPos[0])
					+ (camera.cameraPos[1] * camera.cameraPos[1])
					+ (camera.cameraPos[2] * camera.cameraPos[2])) - EARTH_RADIUS);
			cruise_alt = Math.atan((halfway * Math.tan(setang
					* Math3D.DEGREE_TO_RADIAN_FACTOR))
					/ EARTH_RADIUS);
	...
	protected final void zoomToSelected(MouseEvent e, boolean zoomin) {
		...
		if ((intersectPoint[0] != -999) || (intersectPoint[1] != -999)
			...
			double tx, tz, ty;
			{
				...
				tz = Math.asin(intvec[1]) * Math3D.RADIAN_TO_DEGREE_FACTOR
						* unit.getDD();
				tx = o_lon * unit.getDD();
				...
				if (zoomin) {
					ty /= 4;
					double efloor = landscape.groundHeight(tx, tz, 0)
							/ unit.getCoordSystemRatio() + 50;
					// floor of 100 meters
					if (ty < efloor) {
						ty = efloor;
					}
	...
	private final void setRays(MouseEvent e) {
		...
		double pixelCenterX = screenWidth / 2.0, pixelCenterY = screenHeight / 2.0; // number
																					...
																					// of
																					// viewport
		double worldFOVDistY = Math.tan(Math3D.DEGREE_TO_RADIAN_FACTOR
				* (camera.fov / 2.0)); // world distance from view center to top
										// edge of viewport (on near viewplane)
		double worldFOVDistX = worldFOVDistY
				* (screenWidth / (double) screenHeight); // distance from view
															// center to right
															// edge of viewport
															// (on near
															// viewplane)

		double rightDist = ((e.getX() - pixelCenterX) / pixelCenterX)
				* worldFOVDistX; // horiz distance from view center to clicked
									// point (on near viewplane)
		double upDist = ((pixelCenterY - e.getY()) / pixelCenterY)
				* worldFOVDistY; // vert distance from view center to clicked
									// point (on near viewplane)

		pickray[0][0] = camera.cameraMat.m[0][0] * rightDist
				+ camera.cameraMat.m[0][1] * upDist - camera.cameraMat.m[0][2];
		pickray[0][1] = camera.cameraMat.m[1][0] * rightDist
				+ camera.cameraMat.m[1][1] * upDist - camera.cameraMat.m[1][2];
		pickray[0][2] = camera.cameraMat.m[2][0] * rightDist
