...
public class TextureManager
{
	...
	{
		this.useSubtexturing = Ptolemy3D.getConfiguration().useSubtexturing;

		tileTextureIDs = new int[MapData.MAX_NUM_RESOLUTION][1];
		tileNumTextures = new int[MapData.MAX_NUM_RESOLUTION];
		Arrays.fill(tileNumTextures, 0);
		
		textures = new HashMap<Integer, Integer>();
		emptyRecycler = new Vector<Integer>(20);
		trash = new Vector[MapData.MAX_NUM_RESOLUTION];
...
//			IO.printlnRenderer("Texture deletion: "+textureID);
	...
	public final void destroyTrashTextures(GL gl)
	{
		if(Ptolemy3D.getTileLoader() == null) {
			IO.printError("Can't destroy textures.");
			return;
		}
		Ptolemy3D.getTileLoader().getTextureTrash(trash);
		if (trash == null) {
			return;
		}
