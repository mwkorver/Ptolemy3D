...
//						float landscapPercent = (landscapeProf.duration == 0) ? 0 : (float)renderProf.duration / landscapeProf.duration;
                ...
                case 3:
                    ...
                    switch (Ptolemy3D.getScene().landscape.getDisplayMode()) {
    ...
    public static void drawProfiler(DrawContext drawContext) {
        ...
        if (enablePrintProfiler) {
            ...
            FntFontRenderer fontText = Ptolemy3D.getFontTextRenderer();

            final Sky sky = Ptolemy3D.getScene().sky;
            final Ptolemy3DGLCanvas canvas = drawContext.getCanvas();
            ...
            double aspect = canvas.getDrawAspectRatio();
            double pixratiox = (((Math.tan(Math3D.DEGREE_TO_RADIAN_FACTOR * 60.0) * sclsize) * aspect) / canvas.getDrawWidth());
            ...
            scalerX = (float) scrnsize.width / (float) canvas.getDrawWidth();
            scalerY = (float) scrnsize.height / (float) canvas.getDrawHeight();
            ...
            // bg box
            Color color = Ptolemy3D.getConfiguration().hudBackgroundColor;
