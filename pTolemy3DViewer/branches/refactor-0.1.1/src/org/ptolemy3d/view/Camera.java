...
public class Camera {
	...
	public boolean isTileInView(double lon, double lat, int tileSize) {
		...
		Math3D.setSphericalCoord(lon, -lat, EARTH_RADIUS, point0);
		Math3D.setSphericalCoord(lon, -lat - tileSize, EARTH_RADIUS, point1);
		Math3D.setSphericalCoord(lon + tileSize, -lat - tileSize, EARTH_RADIUS, point2);
		Math3D.setSphericalCoord(lon + tileSize, -lat, EARTH_RADIUS, point3);
	...
	public boolean realPointInView(double lon, double alt, double lat, int MAXANGLE) {
		...
		Math3D.setSphericalCoord(lon, lat, EARTH_RADIUS + alt, coord);
		double angle = Math3D.angle3dvec(
				-cameraMat.m[0][2],
				-cameraMat.m[1][2],
				-cameraMat.m[2][2],
				(cameraPos[0] - coord[0]),
