...
public class XVRMLPlugin implements Plugin
{
		...
		for (i = 0; i < numVObjs; i++)
		{
			...
			if ((xobj != null) && (xobj.lists.length > 0))
			{
				...
				if (obj_tf[i] != null)
				{
					...
					gl.glTranslated(obj_tf[i][4] * unit.getCoordSystemRatio(), obj_tf[i][5] * unit.getCoordSystemRatio(), obj_tf[i][6] * unit.getCoordSystemRatio());
				}
	...
	private final void loadData(boolean force, Communicator JC) throws IOException
	{
		...
		if ((((tx - dispr) < feature_minx) || ((tx + dispr) > feature_maxx) || ((ty + dispr) > feature_maxz) || ((ty - dispr) < feature_minz)) || (force))
		{
			...
			JC.requestData(GetUrl + "&LAYER=" + ENC_LAYER + "&FACTOR=" + (unit.getDD()) + "&BBOX=" + feature_minx + "," + feature_minz + "," + feature_maxx + "," + feature_maxz);
		...
		for (i = 0; i < numVObjs; i++)
		{
			if (obj_vrmlid[i] != b_id)
			{
				...
				if (xvrml_display.get(id_str) == null)
				{
					...
					dist = Math3D.distance3D(position.getLongitudeDD(), obj_x[i],
							0, 0,
							position.getLatitudeDD(), obj_z[i]);
					if (dist < closest)
					{
						closest = dist;
						b_id = obj_vrmlid[i];
