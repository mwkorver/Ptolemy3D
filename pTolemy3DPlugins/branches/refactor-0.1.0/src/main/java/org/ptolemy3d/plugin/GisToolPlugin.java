...
public class GisToolPlugin implements Plugin
{
	...
	public void init(Ptolemy3D ptolemy)
	{
		...
		this.RAISE *= ptolemy.unit.coordSystemRatio;
	}
	...
	public void draw(GL gl)
	{
		...
		if ((mode == 2) && (setRaster))
		{
			...
			if (!useWF)
			{
				...
				raster_tx = Math.min(pt1.rp[0], pt2.rp[0]) / unit.DD;
				raster_ty = Math.max(pt1.rp[1], pt2.rp[1]) / unit.DD;
				raster_sx = ((Math.max(pt1.rp[0], pt2.rp[0]) - Math.min(pt1.rp[0], pt2.rp[0])) / unit.DD) / raster_px;
				raster_sy = ((Math.max(pt1.rp[1], pt2.rp[1]) - Math.min(pt1.rp[1], pt2.rp[1])) / unit.DD) / raster_py;
			}
		}
		...
		if ((mode == 2) && (tex_id > 0))
		{
			...
			gl.glBindTexture(GL.GL_TEXTURE_2D, tex_id);
			{
				...
				double theta2 = (raster_tx + x_w + 180) * Math3D.degToRad;  // startx
				double phi2 = (raster_ty - y_w) * Math3D.degToRad; //starty
	...
	public String pluginAction(String commandname, String command_params)
	{
		...
		else if (commandname.equalsIgnoreCase("addPoint"))
		{
			try
			{
				...
				{
					VectorNode point;
					point = new VectorNode((int) (tx * unit.DD), (int) (ty * unit.DD), raise, true);
					addPoint(point);
				}
			}
			catch (Exception e)
	...
	public boolean onPick(double[] intersectPoint)
	{
		...
		point = new VectorNode((int) (pt[1] * unit.DD), (int) (pt[0] * unit.DD), RAISE, true);
