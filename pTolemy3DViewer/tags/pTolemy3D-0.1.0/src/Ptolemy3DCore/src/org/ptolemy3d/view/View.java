		...
		if (sky.fogStateOn)
		{
			if (camera.getVerticalAltitudeMeters() > sky.horizonAlt) {
				...
				if (gamma > Math3D.halfPI) {
					...
					double max_view = Math.tan(gamma) * (camera.getAltitudeDD() + cameraController.ground_ht) * 2;
					if (max_view >= FOG_RADIUS + 10000) {
						perspective.setPerspectiveProjection(fov, aspectRatio, 1, FOG_RADIUS + 10000);
					}
					else {
						perspective.setPerspectiveProjection(fov, aspectRatio, 1, max_view);
					}
				}
			}
		}
