...
public class BuildingPlugin implements Plugin
{
					...
					(Math3D.distance3D(latLonAlt.getLongitudeDD(), ctds[i][0], latLonAlt.getAltitudeDD(), bu_z[i], latLonAlt.getLatitudeDD(), ctds[i][1]) < drcheck))
			{
				for (j = 0; j < bu_shape[i].length; j++)
				{
					...
					for (k = 0; k < bu_shape[i][j].length; k++)
					{
						...
						{
							repvaly = (bu_z[i] / unit.getCoordSystemRatio()) / tex_h[i];
						}
						...
						for (m = 0; m < bu_shape[i][j][k].length - 1; m++)
						{
							...
							if (hastex)
							{
								...
								{
									repvalx = (Math3D.distance2D(bu_shape[i][j][k][m].rp[0], bu_shape[i][j][k][m + 1].rp[0], bu_shape[i][j][k][m].rp[1], bu_shape[i][j][k][m + 1].rp[1]) / unit.getCoordSystemRatio()) / tex_w[i];
								}
								gl.glTexCoord2d(0, 0);
	...
	private final void loadData(boolean force, Communicator JC) throws IOException
	{
		...
		if ((((tx - dispr) < feature_minx) || ((tx + dispr) > feature_maxx) || ((ty + dispr) > feature_maxz) || ((ty - dispr) < feature_minz)) || (force))
		{
			...
			JC.requestData(GetUrl + "&LAYER=" + ENC_LAYER + "&BBOX=" + feature_minx + "," + feature_minz + "," + feature_maxx + "," + feature_maxz + "&FACTOR=" + String.valueOf(unit.getDD()));
			data = JC.getData();
			...
			for (i = 0; i < numBldgs; i++)
			{
				...
				// ht
				bu_z[i] = ByteReader.readInt(data, cur) * unit.getCoordSystemRatio();
