...
//					int h = (logoMeta[i][1] <= 32) ? logoMeta[i][1] : 32;
	...
	public void draw(GL gl)
	{
		...
		final Ptolemy3DGLCanvas drawable = ptolemy.canvas;
		...
		double pixratiox = (((Math.tan(Math3D.DEGREE_TO_RADIAN_FACTOR * 60.0) * 5) * drawable.getScreenAspectRatio()) / drawable.getScreenWidth());
		double pixratioy = ((Math.tan((Math3D.DEGREE_TO_RADIAN_FACTOR * 60.0)) * 5) / drawable.getScreenHeight());

		float texw, texh;

		double thextan = ((Math.tan((60.0 * Math3D.DEGREE_TO_RADIAN_FACTOR) / 2) * 5) * drawable.getScreenAspectRatio());
		double theytan = Math.tan(Math3D.DEGREE_TO_RADIAN_FACTOR * 30.0) * 5;
		...
		for (int i = 0; i < numLogos; i++)
		{
			...
			switch (orientation[i])
			{
				case 1:
					...
					break;
				case 2:
					ulx = (thextan - (pixratiox * logoMeta[i][0] * drawable.getScreenHeight() / 775.0)) - pixratiox;
					uly = theytan - pixratioy;
					break;
				case 4:
					ulx = (thextan - (pixratiox * logoMeta[i][0] * drawable.getScreenHeight() / 775.0)) - pixratiox;
					uly = -(theytan - (pixratioy * logoMeta[i][1] * (drawable.getScreenHeight() / 770.0))) + pixratioy + 0.15;
					break;
				default: // LLC
					ulx = thextan + pixratiox;
					uly = -(theytan - (pixratioy * logoMeta[i][1] * (drawable.getScreenHeight() / 770.0))) + pixratioy + 0.15;
