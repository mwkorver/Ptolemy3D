...
class MeshStrip {
	...
	private Strip searchPath(DelTriangle node, Strip strip) {
		...
		Strip[] strips = new Strip[node.numpartners];
		...
		for (int i = 0; i < node.numpartners; i++) {
			...
			strips[i] = new Strip(strip);

			if (rot != -1) {
				strips[i].rotateStarter(rot);
			}

			strips[i].append(chk.setOddVertLp(node), chk.id);
			strips[i] = searchPath(chk, strips[i]);
		}
		...
		for (int i = 0; i < node.numpartners; i++) {
			if (strips[i] != null) {
				if (strips[i].numTris > lon_val) {
					lon_id = i;
					lon_val = strips[i].numTris;
				}
			}
		}
		if (lon_id == -1) {
