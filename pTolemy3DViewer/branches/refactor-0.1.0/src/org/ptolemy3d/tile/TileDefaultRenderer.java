...
class TileDefaultRenderer implements TileRenderer {
    ...
    private final void drawSubsection_Tin(int x1, int z1, int x2, int z2) throws Exception {
        ...
        for (int i = 0; i < jtile.tin.nSt.length; i++) {
            ...
            if (DEBUG) {
                ProfilerInterface.vertexCounter += jtile.tin.nSt[i].length;
            }
        }
    }

    private final void drawSubsection_Dem(int x1, int z1, int x2, int z2) throws Exception {
        final Level drawLevel = Ptolemy3D.getScene().landscape.getLevels()[drawZlevel];
        ...
        if (DEBUG) {
            ProfilerInterface.vertexCounter += 2 * (endx - startx) * (endz - startz);
        }
    }
    ...
    private final void drawSubsection(int x1, int z1, int x2, int z2) throws Exception {
        ...
        if (DEBUG) {
            ProfilerInterface.vertexCounter += 2 * n * n;
        }
    }
