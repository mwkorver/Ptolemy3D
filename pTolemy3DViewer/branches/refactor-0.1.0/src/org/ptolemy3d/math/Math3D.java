...
public class Math3D {
	 ...
	 */
	public static final void setSphericalCoord(double lon, double lat,
			double[] dest) {
		final Landscape landscape = Ptolemy3D.ptolemy.scene.landscape;
		final Ptolemy3DUnit unit = Ptolemy3D.ptolemy.unit;

		double toRadiansOverDDBuffer = Math3D.DEGREE_TO_RADIAN_FACTOR
				/ unit.getDD();
	...
	public static boolean isPointInView(double[] point) {
		...
		double dot = (forward[0] * point[0]) + (forward[1] * point[1])
				+ (forward[2] * point[2]);
		if (dot <= 0) {
	...
	public static int rayIntersectTri(double[] dest, double[][] tris) {
		...
		if ((n[0] == 0) && (n[1] == 0) && (n[2] == 0)) { // triangle is
															// degenerate
		...
		double xt = tris[0][0] + (r * dir[0]), yt = tris[0][1] + (r * dir[1]), zt = tris[0][2]
				+ (r * dir[2]);
		...
		if ((dest[0] == -999) && (dest[1] == -999) && (dest[2] == -999)) {
			...
			dest[1] = yt;
			dest[2] = zt;
		} else if (distance3D(tris[0][0], xt, tris[0][1], yt, tris[0][2], zt) < distance3D(
				tris[0][0], dest[0], tris[0][1], dest[1], tris[0][2], dest[2])) {
			dest[0] = xt;
			dest[1] = yt;
			dest[2] = zt;
		} else {
	...
	public static double distance3D(double x1, double x2, double y1, double y2,
			double z1, double z2) {
		double x = (x2 - x1);
		double y = (y2 - y1);
		double z = (z2 - z1);
	...
	// public static boolean lineThroughPoint(short[][] inPoints,float xpt,float
	// ypt,int N)
	// {
	...
	// if(distance2D(inPoints[0][i]+(nrun*j) , xpt ,inPoints[1][i]+(nrise*j),ypt
	// ) < 5 )
	// return true;
	// }
	// }
	...
	public static double dot(double a1, double a2, double a3, double b1,
			double b2, double b3) {
	...
	public static float angle2dvec(double x1, double y1, double x2, double y2,
			boolean degrees) {
		return angle3dvec(x1, y1, 0, x2, y2, 0, degrees);
	}

	public static final void computeFaceNormal(float[] a, float[] b, float[] c,
			float[] norm) {
		...
		// Normalize
		double mag1 = Math.sqrt((norm[0] * norm[0]) + (norm[1] * norm[1])
				+ (norm[2] * norm[2]));
		mag1 = 1 / mag1;
		norm[0] *= mag1;
		norm[1] *= mag1;
	...
	public static final void setMapCoord(double[] in, double[] out) {
		...
		double alt = (Math.sqrt(in[0] * in[0]) + (in[1] * in[1])
				+ (in[2] * in[2]) - EARTH_RADIUS)
	...
	public static boolean realPointInView(double lon, double alt, double lat,
			int MAXANGLE) {
		final Camera camera = Ptolemy3D.ptolemy.camera;
		double angle;
		{
	...
	public static double getScreenScaler(double tx, double ty, double tz,
			int fms) {
		final Camera camera = Ptolemy3D.ptolemy.camera;
