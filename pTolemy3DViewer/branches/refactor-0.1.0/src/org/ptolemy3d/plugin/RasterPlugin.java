...
public class RasterPlugin implements Plugin
{
    ...
    public void setPluginParameters(String param)
	{
		...
		while (stok.hasMoreTokens())
		{
			...
			if (parameters.substring(0, 1).equals("P"))
			{
				...
				while (ptok.hasMoreTokens())
				{
					try
					{
						...
						{
							AboveGround = Integer.parseInt(ptok.nextToken()) * unit.getCoordSystemRatio();
						}
			...
			half_pxw = Scale / 2 * unit.getCoordSystemRatio();
		}
	}
	...
	public synchronized void draw(GL gl)
	{
		...
		if (tex_id != -1)
		{
			...
			{
				double rx = (center_x / unit.getDD()) - (Scale / 2);
				double ry = (center_z / unit.getDD()) + (Scale / 2);

				double theta1 = (rx + 180) * Math3D.DEGREE_TO_RADIAN_FACTOR;  // startx
				double phi1 = (ry) * Math3D.DEGREE_TO_RADIAN_FACTOR; //starty

				double theta2 = (rx + Scale + 180) * Math3D.DEGREE_TO_RADIAN_FACTOR;  // startx
				double phi2 = (ry - Scale) * Math3D.DEGREE_TO_RADIAN_FACTOR; //starty
	...
	private final void loadRaster(GL gl, boolean force) throws Exception
	{
		...
		if ((center_x == position.getLongitudeDD()) && (center_z == position.getLatitudeDD()) && (!force)) {
			return;
		}

		center_x = position.getLongitudeDD();
		center_z = position.getLatitudeDD();

		demRes = 0;
		clicker = 11;

		double tx, tz;
		tx = center_x / unit.getDD();
		tz = center_z / unit.getDD();
	...
	private final void setDEM()
	{
		...
		if ((tex_id == -1) || (!landscape.isTerrainEnabled() && (demRes != 0))) {
			return;
		}
		...
		rx = center_x - (Scale / 2 * unit.getDD());
		ry = center_z + (Scale / 2 * unit.getDD());
		cellw = (Scale * unit.getDD()) / (demRes - 1);
		cellh = -(Scale * unit.getDD()) / (demRes - 1);
	...
	public String pluginAction(String commandname, String command_params)
	{
		...
		{
			return "raster," + STATUS + "," + MapserverUrl + "," + PixelWidth + "," + Scale + "," + (AboveGround / unit.getCoordSystemRatio()) + "," + ((float) transparency / 255) + "," + tran_color[0] + ":" + tran_color[1] + ":" + tran_color[2] + "," + maxDemRes;
		}
