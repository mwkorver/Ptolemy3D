...
public class Sky {
    ...
    private final void initSkyTexture(DrawContext drawContext) {
        ...
        byte[] pixels = new byte[numLevels * 2 * 4];
        ...
        for (i = 0; i < numLevels; i++) {
            for (j = 0; j < 2; j++) {
                for (h = 0; h < 3; h++) {
                    pixels[offset++] = (byte) (botColor[h] + (topColor[h] * i));
                }
                pixels[offset++] = (byte) ((float) (numLevels - i) / (numLevels) * 255);
            }
        }

        skyTexId = Ptolemy3D.getTextureManager().load(gl, new Texture(pixels, numLevels, 2), false);
    }
