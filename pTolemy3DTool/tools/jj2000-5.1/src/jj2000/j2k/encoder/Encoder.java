...
public class Encoder implements Runnable {
	...
	public void run() {
		...
		try {
			...
			if (pl.getParameter("i") == null && bufferedImage == null) {
				error(
						"Mandatory input file is missing (-i option) or use a BufferedImage",
						2);
				return;
			}
			...
			if (pl.getParameter("file_format").equals("on")) {
				...
				if (pl.getParameter("rate") != null
						&& pl.getFloatParameter("rate") != defpl
								.getFloatParameter("rate")) {
			...
			if (pl.getBooleanParameter("lossless")
				...
				throw new IllegalArgumentException("Cannot use '-rate' and "
						+ "'-lossless' option at " + " the same time.");
			...
			// **** ImgReader ****
			String param = pl.getParameter("i");
			ncomp = 0;
			ppminput = false;
			imreadervec = new Vector();
			if (param != null && bufferedImage == null) {
				sgtok = new StringTokenizer(param, ",");
				...
				for (int n = 0; n < nTokens; n++) {
					...
					try {
						if (imreadervec.size() < ncomp) {
							error(
									"With PPM input format only 1 input file can "
											+ "be specified", 2);
							return;
						}
						...
						if (".PGM".equalsIgnoreCase(inext)) { // PGM file
							...
							ncomp += 3;
							/******** START OF UPGRADE *******/
						} else if (".PNG".equalsIgnoreCase(inext)) { // PPM file
							if (ncomp > 0) {
								error("With PNG input format only 1 input "
										+ "file can be specified", 2);
								return;
							}
							imreadervec.addElement(new ImgReaderPNG(infile));
							ppminput = true;
							ncomp += 3;
							/******** END OF UPGRADE *******/
						} else { // Should be PGX
						...
						error("Could not open or read from file "
								+ infile
								+ ((e.getMessage() != null) ? (":\n" + e
										.getMessage()) : ""), 3);
						if (pl.getParameter("debug").equals("on")) {
							...
							while (imreadervec.size() > 0) {
								try {
									((ImgReader) imreadervec
											.elementAt(imreadervec.size() - 1))
											.close();
									imreadervec.removeElementAt(imreadervec
											.size() - 1);
								} catch (Exception e) {
								}
							}
				...
				imreadervec.addElement(new ImgReaderPNG(bufferedImage));
				ppminput = true;
				ncomp += 3;
			}
			...
			stok = new StreamTokenizer(new StringReader(pl
					.getParameter("tiles")));
			...
			try {
				...
				throw new IllegalArgumentException("Error while parsing 'ref' "
						+ "option");
			} catch (NumberFormatException e) {
			...
			try {
				...
				throw new IllegalArgumentException(
						"Error while parsing 'tref' " + "option");
			} catch (NumberFormatException e) {
				throw new IllegalArgumentException("Invalid number type in "
						+ "'tref' option");
			}
			...
			try {
				ecoder = EntropyCoder.createInstance(rois, pl, encSpec.cblks,
						encSpec.pss, encSpec.bms, encSpec.mqrs, encSpec.rts,
						encSpec.css, encSpec.sss, encSpec.lcs, encSpec.tts);
			} catch (IllegalArgumentException e) {
			...
			// **** HeaderEncoder ****
			headenc = new HeaderEncoder(imgsrc, imsigned, dwt, imgtiler,
					encSpec, rois, ralloc, pl);
			...
			if (pktspertp > 0 || pphTile || pphMain) {
				...
				try {
					CodestreamManipulator cm = new CodestreamManipulator(
							outname, ntiles, pktspertp, pphMain, pphTile,
							tempSop, tempEph);
					fileLength += cm.doCodestreamManipulation();
					String res = "";
					if (pktspertp > 0) {
						FacilityManager.getMsgLogger().println(
								"Created tile-parts " + "containing at most "
										+ pktspertp + " packets per tile.", 4,
								6);
					}
					if (pphTile) {
						FacilityManager.getMsgLogger().println(
								"Moved packet headers " + "to tile headers", 4,
								6);
					}
					if (pphMain) {
						FacilityManager.getMsgLogger().println(
								"Moved packet headers " + "to main header", 4,
								6);
					}
				} catch (IOException e) {
