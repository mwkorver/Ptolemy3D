...
public class Level {
	...
	public final static int LEVEL_NUMTILES = LEVEL_NUMTILE_LON
			* LEVEL_NUMTILE_LAT;
	 ...
	 * @param divider
	 */
	public Level(int levelId, int minZoom, int maxZoom, int tileSize,
			int divider) {
			...
			for (int j = 0; j < LEVEL_NUMTILE_LON; j++) {
				...
				try {
					tile.display(gl, hasTexture,
							...
							(j == 0) ? null : tiles[(i * LEVEL_NUMTILE_LON) + j
									- 1], (i == 0) ? null
									: tiles[((i - 1) * LEVEL_NUMTILE_LON) + j]);
				} catch (Exception e) {
		...
		if (head.dem != null) {
			...
			Math3D
					.setTriVert(
							0,
							gminx,
							(dem[(ypos * rowWidth) + (xpos * 2)] << 8)
									+ (dem[((ypos * rowWidth) + (xpos * 2)) + 1] & 0xFF),
							gminy);
			Math3D.setTriVert(1, gmaxx, (dem[(ypos * rowWidth)
					+ ((xpos + 1) * 2)] << 8)
					+ (dem[((ypos * rowWidth) + ((xpos + 1) * 2)) + 1] & 0xFF),
					gminy);
			Math3D.setTriVert(2, gminx, (dem[((ypos + 1) * rowWidth)
					+ (xpos * 2)] << 8)
					+ (dem[(((ypos + 1) * rowWidth) + (xpos * 2)) + 1] & 0xFF),
					gmaxy);
			if (Math3D.rayIntersectTri(pickArr, ray) != 1) {
				...
				Math3D
						.setTriVert(
								2,
								gmaxx,
								(dem[((ypos + 1) * rowWidth) + ((xpos + 1) * 2)] << 8)
										+ (dem[(((ypos + 1) * rowWidth) + ((xpos + 1) * 2)) + 1] & 0xFF),
								gmaxy);
				if (Math3D.rayIntersectTri(pickArr, ray) != 1) {
					pickArr[1] = 0;
				}
			...
			for (int k = 0; (k < head.tin.nSt.length); k++) {
				...
				for (int j = 2; (j < head.tin.nSt[k].length); j++) {
					Math3D.setTriVert(0, head.lon
							+ (head.tin.p[head.tin.nSt[k][j - 2]][0]),
							head.tin.p[head.tin.nSt[k][j - 2]][1], head.lat
									- (head.tin.p[head.tin.nSt[k][j - 2]][2]));
					Math3D.setTriVert(1, head.lon
							+ (head.tin.p[head.tin.nSt[k][j - 1]][0]),
							head.tin.p[head.tin.nSt[k][j - 1]][1], head.lat
									- (head.tin.p[head.tin.nSt[k][j - 1]][2]));
					Math3D.setTriVert(2, head.lon
							+ (head.tin.p[head.tin.nSt[k][j]][0]),
							head.tin.p[head.tin.nSt[k][j]][1], head.lat
									- (head.tin.p[head.tin.nSt[k][j]][2]));
					if (Math3D.rayIntersectTri(pickArr, ray) == 1) {
						pickArr[1] *= unit.getCoordSystemRatio();
						return pickArr;
					}
