...
public class Jp2TileLoader implements Runnable {
	...
	public Jp2TileLoader(Ptolemy3D ptolemy) {
		...
		final int numLevels = ptolemy.scene.landscape.getLevels().length;
	...
	protected final void setServers() {
			...
			try {
				if (ptolemy.configuration.locations[i] != null) {
					...
					for (int h = 0; h < ptolemy.configuration.locations[i].length; h++) {
						JC[i].requestMapData(
								ptolemy.configuration.locations[i][h]
										+ "default.jp2", 0,
		...
		if (repHeader == null) {
			...
			targetData = new short[NUMCOMPS][repHeader.getImageW()
					* repHeader.getImageH()];
			...
			for (int res = 0, j = Jp2Tile.NUM_RESOLUTION - 1; res < Jp2Tile.NUM_RESOLUTION; res++, j--) {
				...
				twidth[res] = nd_w;
				quadtiles[res] = (repHeader.getImageW() / nd_w)
						* (repHeader.getImageW() / nd_w);
			}
...
//			}
			...
			try {
				if (!setData(1) && !setData(0)
						&& (!ptolemy.cameraController.isActive)
	...
	private final boolean setData(int getVisible) throws IOException {
		final Level[] levels = ptolemy.scene.landscape.getLevels();
		...
		for (int p = 0; p < levels.length; p++) {
			...
			if (tileId != -1) {
				...
				while ((data == null) && (jp2DatServer < JC.length)) {
					if ((JC[jp2DatServer] == null)
							|| (ptolemy.configuration.locations[jp2DatServer] == null)) {
						jp2DatServer++;
						continue;
					}
					...
					JC[jp2DatServer]
							.requestMapData(
									ptolemy.configuration.locations[jp2DatServer][locIndex]
											+ jp2Tiles[p][tileId].fileBase
											+ ".jp2", 0,
									Communicator.HEADERMAXLEN);
					int ret = JC[jp2DatServer].getHeaderReturnCode();
					if ((ret == 200) || (ret == 206)) {
						data = JC[jp2DatServer]
								.getData(Communicator.HEADERMAXLEN + 1);
					} else {
				...
				if (data != null) {
					try {
						masterH[jp2Tiles[p][tileId].tileDataServer][jp2Tiles[p][tileId].tileDataLoc]
								.setTileHeader(jp2Blocks[p][tileId], data);
						jp2Tiles[p][tileId].gotten = true;
						jp2Tiles[p][tileId].hasData = true;
					} catch (Exception ex) {
						IO.printStackConnection(ex);
						System.err.println("TileLoader.setData(): "
								+ ex.toString());
						System.err
								.println("File: "
										+ ptolemy.configuration.locations[jp2DatServer][locIndex]
										+ jp2Tiles[p][tileId].fileBase + ".jp2");
					}
				} else {
					jp2Tiles[p][tileId].gotten = true;
				}
			...
			if (tileId != -1) {
				...
				while ((!demOrTinSet) && (jp2DatServer < JC.length)) {
					if ((JC[jp2DatServer] == null)
							|| (ptolemy.configuration.DEMLocation[jp2DatServer] == null)) {
						jp2DatServer++;
						continue;
					}
					int ret = -1;
					...
					if (USE_TIN) {
						JC[jp2DatServer]
								.requestMapData(ptolemy.configuration.DEMLocation[jp2DatServer][locIndex]
										+ "tin/"
										+ jp2Tiles[p][tileId].fileBase
										+ ".tin");
					...
					if ((ret != 200) && (ret != 206)) {
						...
						JC[jp2DatServer]
								.requestMapData(ptolemy.configuration.DEMLocation[jp2DatServer][locIndex]
										+ jp2Tiles[p][tileId].fileBase + ".bdm");
						isTin = false;
						ret = JC[jp2DatServer].getHeaderReturnCode();
					}
			...
			for (res = 0; res < Jp2Tile.NUM_RESOLUTION; res++) {
				...
				if ((res > 0) && (tileId != -1)) {
					...
					for (int j = 0; j < jp2Tiles[p].length; j++) {
						...
						if (tileLevel.tileGotten) {
							...
							dist = getDistanceTileCenter(ptolemy.camera
									.getCameraX(), ptolemy.camera.getCameraY(),
									p, j);
							if (dist > furthest) {
								furthest = dist;
								fix = j;
				...
				if (tileId != -1) {
					boolean isSet = false;
					if ((!jp2Tiles[p][tileId].hasData)
							&& (ptolemy.configuration.backgroundImageUrl != null)) {
						...
						while ((data == null) && (jp2DatServer < JC.length)) {
							...
							JC[jp2DatServer].requestMapData(jp2Tiles[p][tileId]
									.fillFile(res));
						...
						jp2Blocks[p][tileId]
								.requestMapData(
										JC[dataServerId],
										ptolemy.configuration.locations[dataServerId][dataLocId]
												+ jp2Tiles[p][tileId].fileBase
												+ ".jp2", res);
						...
						if (data != null) {
							...
							if (res > 0) {
								...
								int prevRes = res - 1;
								byte[] imgData = jp2Tiles[p][tileId]
										.getImageData(prevRes);
								if (imgData == null) {
	...
	private final int getClosestTile(int type, int lvl, int isVisible, int res) {
		...
		for (int j = 0; j < jp2Tiles[lvl].length; j++) {
			...
			case 1: // header
				...
				break;
			case 2: // jp2
				if ((!tile.tileRes[res].tileGotten)
						&& ((tile.hasData) || ((ptolemy.configuration.backgroundImageUrl != null)
								&& (res <= 2) && (tile.gotten)))) {
					if ((tile.terrainGotten > 0)
							|| (!landscape.isTerrainEnabled())) {
						doCheck = true;
	...
	private double getDistanceTileCenter(double x, double z, int p, int j) {
		...
		if (x < 0) {
			tx = x + (landscape.getMaxLongitude() - jp2Tiles[p][j].lon)
					+ (x + landscape.getMaxLongitude());
		} else {
			tx = x + (landscape.getMaxLongitude() - x)
					+ (jp2Tiles[p][j].lon + landscape.getMaxLongitude());
		}
	...
	private boolean tileInView(int lvl, int j) {
		final Level level = ptolemy.scene.landscape.getLevels()[lvl];
		...
		{
			// above ?
			if ((latLonAlt.getLongitudeDD() >= jp2Tiles[lvl][j].lon)
					&& (latLonAlt.getLongitudeDD() <= jp2Tiles[lvl][j].lon
							+ level.getTileSize())
					&& (latLonAlt.getLatitudeDD() <= jp2Tiles[lvl][j].lat)
					&& (latLonAlt.getLatitudeDD() >= jp2Tiles[lvl][j].lat
							- level.getTileSize())) {
				return true;
			}
			...
			Math3D
					.setSphericalCoord(
							((latLonAlt.getLongitudeDD() < jp2Tiles[lvl][j].lon) ? jp2Tiles[lvl][j].lon
									: (latLonAlt.getLongitudeDD() > (jp2Tiles[lvl][j].lon + level
											.getTileSize())) ? (jp2Tiles[lvl][j].lon + level
											.getTileSize())
											: latLonAlt.getLongitudeDD()),
							((latLonAlt.getLatitudeDD() > jp2Tiles[lvl][j].lat) ? jp2Tiles[lvl][j].lat
									: (latLonAlt.getLatitudeDD() < (jp2Tiles[lvl][j].lat - level
											.getTileSize())) ? (jp2Tiles[lvl][j].lat - level
											.getTileSize())
	...
	private boolean checkInScene(int p, double mx, double mz) {
		final Level level = ptolemy.scene.landscape.getLevels()[p];

		final Jp2Tile[] tiles = jp2Tiles[p];
		for (int q = 0; q < tiles.length; q++) {
			final Jp2Tile tile = tiles[q];
			if ((tile.scale == level.getTileSize()) && (tile.lon == mx)
					&& (tile.lat == mz)) {
				tile.inScene = true;
				return true;
			}
		...
		for (int p = 0; p < levels.length; p++) {
			...
			for (int k = 0; k < level.getTiles().length; k++) {
				if (jp2Tiles[p][k] == null) {
					...
					jp2Tiles[p][k] = new Jp2Tile(p, level.getTileSize(), level
							.getTiles()[k].getFmx(), level.getTiles()[k].getFmz());
					jp2Blocks[p][k] = new Jp2Block();
					jp2Blocks[p][k].fileBase = jp2Tiles[p][k].fileBase;
					found[k] = true;
				} else {
					found[k] = checkInScene(p, level.getTiles()[k].getFmx(), level
							.getTiles()[k].getFmz());
				}
			}
				...
				if (!found[k]) {
					for (; q < jp2Tiles[p].length; q++) {
						...
						if (!jp2Tile.inScene) {
							...
							jp2Tile
									.set(p, level.getTileSize(), level
											.getTiles()[k].getFmx(), level
											.getTiles()[k].getFmz());
	...
	protected final Jp2Tile getHeader(double x, double z, int lvl) {
		...
		for (int m = 0; m < jp2Tiles[lvl].length; m++) {
			if ((jp2Tiles[lvl][m] != null) && (jp2Tiles[lvl][m].lon <= x)
					&& ((jp2Tiles[lvl][m].lon + level.getTileSize()) > x)
					&& (jp2Tiles[lvl][m].lat >= z)
					&& ((jp2Tiles[lvl][m].lat - level.getTileSize()) < z)) {
	...
	public Jp2Tile getJp2Tile(double lon, double lat, boolean all) {
		...
		for (int m = 0; m < jp2Tiles[lvl].length; m++) {
			if ((jp2Tiles[lvl][m] != null) && (jp2Tiles[lvl][m].lon <= lon)
					&& ((jp2Tiles[lvl][m].lon + level.getTileSize()) > lon)
					&& (jp2Tiles[lvl][m].lat >= lat)
					&& ((jp2Tiles[lvl][m].lat - level.getTileSize()) < lat)
	...
	protected Jp2Tile getAlternateData(Tile tile) {
		...
		while (true) {
			...
			for (int m = 0; m < jp2Tiles[below].length; m++) {
				if ((jp2Tiles[below][m] != null)
						&& (jp2Tiles[below][m].lon <= x)
						&& ((jp2Tiles[below][m].lon + levelBelow.getTileSize()) > x)
						&& (jp2Tiles[below][m].lat >= z)
						&& ((jp2Tiles[below][m].lat - levelBelow.getTileSize()) < z)) {
	...
	public void setServerHeaderKey(int server_id, String key) {
		if ((server_id > 0) && (server_id < JC.length)
				&& (JC[server_id] != null)) {
	...
	public void acquireTile(Level level, boolean lastLevel) {
		...
		for (int i = 0; i < Level.LEVEL_NUMTILES; i++) {
			...
			if ((tile.jp2 == null)
					|| ((tile.isImageDataReady(0) == false) && !lastLevel)) {
