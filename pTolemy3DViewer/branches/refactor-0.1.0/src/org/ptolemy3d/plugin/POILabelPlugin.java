...
public class POILabelPlugin implements Plugin
{
    ...
    public synchronized void draw(GL gl)
    {
    	...
    	if ((numShowableFeatures > 0) && (features_id != -1))
    	{
    		...
    		for (int i = 0; i < numFeatures; i++)
    		{
    			...
    			{
    				cp_Y = landscape.groundHeight(feature_real_coord[i][0], feature_real_coord[i][1], 0) + (raise * unit.getCoordSystemRatio());
    		...
    		for (int i = 0; i < numFeatures; i++)
    		{
    			if (feature_text_ix[i] != -1)
    			{
    				...
    				ty = landscape.groundHeight(feature_real_coord[i][0], feature_real_coord[i][1], 0) + (raise * unit.getCoordSystemRatio());
    				...
    				if (pinpoint)
    				{
    					...
    					{
    						gl.glVertex3d(obj_end[i][0] * (EARTH_RADIUS + (ty - (raise * unit.getCoordSystemRatio()))),
    								obj_end[i][1] * (EARTH_RADIUS + (ty - (raise * unit.getCoordSystemRatio()))),
    								obj_end[i][2] * (EARTH_RADIUS + (ty - (raise * unit.getCoordSystemRatio()))));
    					}
    					gl.glEnd();
    					gl.glEnable(GL.GL_TEXTURE_2D);
    				}
    ...
    private final void drawDistLine(GL gl, double px, double py, double pz, double cp_X, double cp_Y, double cp_Z)
    {
    	...
    	byte[] rw_dist = String.valueOf((int) ((dist / unit.getCoordSystemRatio()) * 10)).getBytes();
    ...
    public boolean pick(double[] intpt, double[][] ray)
    {
    	...
    	if (numShowableFeatures > 0)
    	{
    		...
    		for (int i = 0; i < numFeatures; i++)
    		{
    			...
    			ht = landscape.groundHeight(feature_real_coord[i][0], feature_real_coord[i][1], 0) + (raise * unit.getCoordSystemRatio());
    ...
    private final void drawHighlight(GL gl, int hl_id)
    {
    	...
    	switch (hl_id)
    	{
    		...
    		case HLT_RINGS:
    			int r_speed = 30;
    			double step = (Math3D.TWO_PI_VALUE) / r_speed;
	...
	public final void loadFeaturesWFS(boolean force, Communicator JC) throws IOException{
		...
		final Camera camera = ptolemy.camera;
		final Position position = camera.getPosition();
		
		int tx,ty;
		tx = (int)position.getLongitudeDD();
		ty = (int)position.getLatitudeDD();
    ...
    public final void loadFeaturesServlet(boolean force, Communicator JC) throws IOException
    {
    	...
    	if (((area_minx < feature_minx) || (area_maxx > feature_maxx) || (area_maxz > feature_maxz) || (area_minz < feature_minz)) || (force))
    	{
    		...
    		String request = IconGetUrl + "&LAYER=" + enc_layer + "&BBOX=" + feature_minx + "," + feature_minz + "," + feature_maxx + "," + feature_maxz + "&FACTOR=" + String.valueOf(unit.getDD());
    ...
    public final void prepareFeatures(boolean force)
    {
    	...
    	if ((position.getLongitudeDD() == feature_center_x) && (position.getLatitudeDD() == feature_center_z) && (!force)) {
    		return;
    	}

    	feature_center_x = position.getLongitudeDD();
    	feature_center_z = position.getLatitudeDD();
