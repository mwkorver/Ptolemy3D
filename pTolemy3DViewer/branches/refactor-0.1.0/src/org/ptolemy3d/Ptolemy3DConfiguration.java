...
public class Ptolemy3DConfiguration {
	...
	public void loadSettings(Element docelem) throws Ptolemy3DException {
		...
		try {
			...
			if (orientation != null) {
				...
				if (values.length == 5) {
					try {
						...
						cameraController.setOrientation(Position.fromDD(lat,
								lon, alt), dir, pitch);
			...
			flightFrameDelay = getOptionalParameterInt(FLIGHT_FRAME_DELAY,
					docelem);
			...
			if (maxalt != -1) {
				cameraController.maxAlt = maxalt;
			}
			...
			for (int p = 0; p < numPlugins; p++) {
				try {
					plugins.addPlugin(getOptionalParameter(PLUGIN_TYPE
							+ (p + 1), docelem), getOptionalParameter(
...
//		}
	...
	private String getRequieredParameter(String name, Element elem)
		...
		try {
			...
			parameter = ((Element) nodeList.item(0)).getChildNodes().item(0)
					.getNodeValue();
		} catch (Exception ex) {
			parameter = null;
			String message = "The requiered parameter '" + name
