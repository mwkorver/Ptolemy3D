...
//	}
	 ...
	 */
	public static final void setSphericalCoord(double lon, double lat, double[] a)
	{
		...
		double thx = (lon - landscape.maxLongitude) * toRadiansOverDDBuffer;	//TODO + or -
	...
	public static boolean realPointInView(double x, double y, double z, int MAXANGLE)
	{
		final Camera camera = Ptolemy3D.ptolemy.camera;
		double angle;
		{
			double[] coord = new double[3];
			setSphericalCoord(x, z, coord);
			x = coord[0] * (EARTH_RADIUS + y);
			y = coord[1] * (EARTH_RADIUS + y);
			z = coord[2] * (EARTH_RADIUS + y);
			angle = Math3D.angle3dvec(-camera.cameraMat.m[0][2], -camera.cameraMat.m[1][2], -camera.cameraMat.m[2][2],
					(camera.cameraPos[0] - x), (camera.cameraPos[1] - y), (camera.cameraPos[2] - z),
					true);
		}
