...
/**
 * Configuration class loads and stores a set of Ptolemy3D configuration
 * parameters. This is used for example to specify the map server date, globe
 * geometry settings (number of levels)), the initial view position, etc.
 * 
...
public class Configuration {
	...
	/**
	 * Constructs a new Configuration taken values from the applet's parameters.
	 * <b>Only use this constructor into applets otherwise it will fail.</b>
	 */
	...
	public void loadSettings(Element docelem) throws Ptolemy3DException {
		...
		// MapDataServer
		int nDataServers = getRequieredParameterInt(Requiered.NumMapDataServer,
				docelem);
		servers = new ServerConfig[nDataServers];
		for (int i = 1; i <= nDataServers; i++) {
			String dataServers = getRequieredParameter(
					Requiered.MapDataSvr + i, docelem);
			String headerKeys = getOptionalParameter(Optional.MHeaderKey + i,
					docelem);
			String urlAppends = getOptionalParameter(Optional.MUrlAppend + i,
					docelem);

			String[] locations;
			String temppm = getRequieredParameter(Requiered.MapJp2Store + i,
					docelem);
			if (temppm == null) {
				locations = null;
			} else {
			...
			servers[i - 1] = new ServerConfig(headerKeys, dataServers,
					locations, DEMLocation, keepAlives, urlAppends);
		}

		try {
			// Layers: Landscape levels (resolution levels) and layers
			// properties
			final int numLayers = getRequieredParameterInt(Requiered.NumLayers,
					docelem);
			final Layer[] layers = new Layer[numLayers];
			landScape.globe.setLevels(layers);

			for (int i = 0; i < numLayers; i++) {
				int tilePixel = getRequieredParameterInt(Requiered.LayerWidth_
						+ (i + 1), docelem);
				int maxZoom = getRequieredParameterInt(Requiered.LayerMax_
						+ (i + 1), docelem);
				int divider = getOptionalParameterInt(Optional.LayerDivider_
						+ (i + 1), docelem);
				if (divider == -1) {
					divider = 0;
				}
		...
		try {
			// Initial position of the camera.
			String orientation = getOptionalParameter(Optional.Orientation,
					docelem);
			if (orientation != null) {
				...
				if (values.length == 5) {
					try {
						...
						String message = "Some of the values in the "
								+ Optional.Orientation
								+ " property are invalids.\nThe property will be ignored";
						logger.warning(message);
					}
				} else {
					String message = "The number of values for the "
							+ Optional.Orientation
							+ " property does not correspond to (lat,lon,alt,dir,pitch). "
							+ "You have specified a different number.\nThe property will be ignored";
					logger.warning(message);
				}
			}
		} catch (Ptolemy3DConfigurationException ex) {
		...
		try {
			...
			flightFrameDelay = getOptionalParameterInt(
					Optional.FlightFrameDelay, docelem);
			...
			// Background image
			backgroundImageUrl = getOptionalParameter(
					Optional.BackgroundImageUrl, docelem);
			...
			if ((s = getOptionalParameter(Optional.TileColor, docelem)) != null) {
				...
				if (st.countTokens() > 2) {
					landScape.getTileColor()[0] = (Float.parseFloat(st
							.nextToken()) / 255);
					landScape.getTileColor()[1] = (Float.parseFloat(st
							.nextToken()) / 255);
					landScape.getTileColor()[2] = (Float.parseFloat(st
							.nextToken()) / 255);
				}
			}
		...
		// Plugins
		try {
			int numPlugins = getOptionalParameterInt(Optional.NumPlugins,
					docelem);
			for (int p = 0; p < numPlugins; p++) {
				try {
					String pluginName = getOptionalParameter(
							Optional.PluginType_ + (p + 1), docelem);
					String pluginParams = getOptionalParameter(
							Optional.PluginParam_ + (p + 1), docelem);

					plugins.addPlugin(pluginName, pluginParams);
				} catch (Exception ex) {
